...
function AdminPaymentTest() {
  ...
  const verifyPhonePePayment = async (orderId, isManual = false) => {
    try {
      ...
      try {
        ...
        if (response.data) {
          if (response.data.paymentStatus === 'success') {
            ...
            setPhonePeStatus('success');
            setShowPaymentChannel(false);
            setPaymentStatus(`✅ Payment successful! Invoice #${invoiceData?.invoiceNumber || 'generated'}.`);
            ...
            setPaymentStatus(`⏳ Waiting for payment confirmation... (${verificationAttempts + 1}/10)`);
        ...
        if (apiError.response?.status === 404 || apiError.message?.includes('Cannot POST')) {
          ...
          if (verificationAttempts >= 10) {
            setVerifyingPayment(false);
            setPaymentStatus('⏳ Payment still processing. Please check manually.');
            setShowRetryButton(true);
            return { success: false, status: 'pending' };
          }
      ...
      if (isManual) {
        setPaymentStatus('❌ Failed to verify payment: ' + (error.message || 'Unknown error'));
        setShowRetryButton(true);
      }
  ...
  const manuallyConfirmPayment = async () => {
    ...
    try {
      ...
      setShowPaymentChannel(false);
      stopPolling();
      setPaymentStatus(`✅ Payment confirmed manually! Invoice #${invoiceData?.invoiceNumber || 'generated'}.`);
      ...
      setPaymentStatus('❌ Failed to confirm payment: ' + error.message);
      setVerifyingPayment(false);
  ...
  const openPaymentChannel = (url) => {
    if (url) {
      ...
      setPhonePeStatus('pending');
      setPaymentStatus('🔄 Payment channel opened. Complete the payment in PhonePe...');
    }
  };
  ...
  const processPayment = async () => {
    ...
    try {
      ...
      if (selectedPaymentMethod === 'cash') {
        ...
        setPaymentCompleted(true);
        setPaymentConfirmed(true);
        setPaymentStatus(`✅ Cash payment successful! Invoice #${invoiceData?.invoiceNumber || 'generated'}.`);
        setLoading(false);
        return;
      }
      ...
      if (selectedPaymentMethod === 'card') {
        ...
        setPaymentCompleted(true);
        setPaymentConfirmed(true);
        setPaymentStatus(`✅ Card payment successful! Invoice #${invoiceData?.invoiceNumber || 'generated'}.`);
        setLoading(false);
        return;
      }
      ...
      if (!paymentConfirmed) {
        ...
        if (response.status === 200 && response.data.result) {
          ...
          setPaymentStatus('🔄 Payment initiated! Click "Open Payment Channel" to complete payment in PhonePe...');
          ...
          pollIntervalRef.current = setInterval(async () => {
            ...
            if (pollCount > maxPolls) {
              ...
              setVerifyingPayment(false);
              setLoading(false);
              setPaymentStatus('⏳ Payment verification timed out. Please check status manually.');
              setShowRetryButton(true);
              return;
            }
            ...
            if (dbStatus && dbStatus.payment_status === 'paid') {
              ...
              setShowPaymentChannel(false);
              stopPolling();
              setPaymentStatus(`✅ Payment confirmed! Invoice #${invoiceData?.invoiceNumber || 'generated'}.`);
          ...
          timeoutRef.current = setTimeout(() => {
            ...
            if (!isSuccess && loading) {
              setVerifyingPayment(false);
              setLoading(false);
              setPaymentStatus('⏳ Payment is taking longer than expected. Please check your PhonePe app.');
              setShowRetryButton(true);
            }
          }, 300000);
  ...
  const handleRetryVerification = async () => {
    ...
    if (result.success) {
      ...
      setVerifyingPayment(false);
      setPhonePeStatus('pending');
      setPaymentStatus('⏳ Payment still pending. Try manual confirmation if payment was completed.');
      setShowRetryButton(true);
    }
  };
          ...
          <div className="lg:col-span-2">
            ...
            >
              <div className="flex items-center gap-4 flex-wrap">
                <label className="font-semibold text-gray-700">📍 Outlet:</label>
                <select 
                  value={outletId} 
                  onChange={(e) => {
                            ...
                            return (
                              ...
                              >
                                <div className="flex-1">
                                  ...
                                  {isDisabled && (
                                    <div className="text-xs text-red-500 mt-1">⚠️ No available variants</div>
                                  )}
                                </div>
                                <motion.button
                                  whileTap={{ scale: 0.9 }}
