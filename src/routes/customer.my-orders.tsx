...
export const Route = createFileRoute("/customer/my-orders")({
  head: () => ({ meta: [{ title: "My Orders — Kosia Rajula Ruchulu" }] }),
              ...
              return (
                <div key={o.id} className="rounded-2xl border border-gold/25 bg-card p-3 shadow-sm">
                  ...
                  <div className="mt-3 grid grid-cols-4 gap-1.5 text-[10px] font-semibold">
                    <button onClick={() => navigate({ to: "/customer/order-detail/$orderId", params: { orderId: o.id } })}
                    ...
                    <button onClick={() => { toast.message("Reorder will be available after cart validation is connected."); navigate({ to: "/customer/menu" }); }}
