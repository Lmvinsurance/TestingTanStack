...
export const Route = createFileRoute("/payment-status")({
  head: () => ({ meta: [{ title: "Payment Status — Kosia Rajula Ruchulu" }] }),
  validateSearch: (s: Record<string, unknown>): Search => ({
    orderId: typeof s.orderId === "string" ? s.orderId : undefined,
    method: (["phonepe", "upi", "cod"].includes(String(s.method)) ? s.method : undefined) as Search["method"],
  }),
...
function PaymentStatusScreen() {
  ...
  useEffect(() => {
    if (!ready) return;
    if (!orderId) { navigate({ to: "/cart", replace: true }); return; }
    (async () => {
      const d = await refresh();
    ...
    <main className="min-h-screen bg-cream pb-20">
      ...
      <div className="mx-auto max-w-md space-y-5 px-4 py-6 sm:max-w-lg">
        ...
        <div className="grid grid-cols-2 gap-2">
          <button onClick={() => navigate({ to: "/customer/order-detail/$orderId", params: { orderId: order.order.id } })}
