...
export const Route = createFileRoute("/checkout")({
  head: () => ({ meta: [{ title: "Checkout — Kosia Rajula Ruchulu" }] }),
...
function CheckoutScreen() {
  ...
  useEffect(() => {
    if (!ready) return;
    if (!outlet) { navigate({ to: "/customer/outlets", replace: true }); return; }
    if (cart.length === 0) { navigate({ to: "/cart", replace: true }); return; }
    if (mismatch) { navigate({ to: "/cart", replace: true }); }
